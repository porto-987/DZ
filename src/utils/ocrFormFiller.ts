...
export function extractLegalTextData(ocrText: string): Partial<LegalTextFormData> {
  logger.debug('OCR', 'Extraction du texte', { textLength: ocrText.length, preview: ocrText.substring(0, 200) + '...' }, 'OCRFormFiller');
  ...
  if (typeMatches && typeMatches.length > 0) {
    ...
    logger.info('OCR', 'Type détecté', { detectedType });
  ...
  if (authorityMatches && authorityMatches.length > 0) {
    ...
    logger.info('OCR', 'Autorité détectée', { authority: data.authority });
  }
  ...
  if (domainMatches && domainMatches.length > 0) {
    ...
    logger.info('OCR', 'Domaine détecté', { category: data.category });
  }
  ...
  if (titleMatches && titleMatches.length > 0) {
    ...
    logger.info('OCR', 'Titre détecté', { title: data.title });
  }
  ...
  if (headerMatches && headerMatches.length > 0) {
    ...
    logger.info('OCR', 'En-tête détecté', { en_tete: data.en_tete });
  }
  ...
  if (descriptionMatches && descriptionMatches.length > 0) {
    ...
    logger.info('OCR', 'Description détectée', { description: data.description });
  }
  ...
  if (signatoryMatches && signatoryMatches.length > 0) {
    ...
    logger.info('OCR', 'Signataire détecté', { authority: data.authority });
  }
  ...
  if (languageMatches && languageMatches.length > 0) {
    ...
    logger.info('OCR', 'Langue détectée', { language: data.language });
  }
  ...
  if (refMatches && refMatches.length > 0) {
    ...
    logger.info('OCR', 'Référence détectée', { reference: data.reference });
  }
  ...
  if (journalMatches && journalMatches.length > 0) {
    ...
    if (match) {
      ...
      logger.info('OCR', 'Journal N° détecté', { journal_numero: data.journal_numero });
    }
  }
  ...
  if (pageMatches && pageMatches.length > 0) {
    ...
    if (match) {
      ...
      logger.info('OCR', 'Page détectée', { numero_page: data.numero_page });
    }
  }
    ...
    try {
      ...
      if (parts.length === 3) {
        ...
        if (!isNaN(date.getTime())) {
          ...
          logger.info('OCR', 'Date détectée', { date_journal: data.date_journal });
        }
      }
    } catch (e) {
  ...
  if (!data.title) {
    ...
    if (lines.length > 0) {
      data.title = lines[0];
      logger.info('OCR', 'Titre par défaut détecté', { title: data.title });
    }
  }
  ...
  logger.info('OCR', 'Données extraites finales', { data });
  return data;
}

export function extractProcedureData(ocrText: string): Partial<ProcedureFormData> {
  logger.info('OCR', 'Extraction OCR procédure du texte', { ocrText: ocrText.substring(0, 200) });
  ...
  if (typeMatches && typeMatches.length > 0) {
    const detectedType = typeMatches[0].toLowerCase();
    logger.info('OCR', 'Type de procédure détecté', { detectedType });
  ...
  if (sectorMatches && sectorMatches.length > 0) {
    ...
    logger.info('OCR', 'Secteur détecté', { detectedSector });
  ...
  if (titleMatches && titleMatches.length > 0) {
    ...
    if (match) {
      ...
      logger.info('OCR', 'Nom de procédure détecté', { name: data.name });
    }
  }
  ...
  if (!data.name) {
    ...
    if (lines.length > 0) {
      data.name = lines[0];
      logger.info('OCR', 'Nom par défaut', { name: data.name });
    }
  }
  ...
  logger.info('OCR', 'Données de procédure extraites', { data });
  return data;
}
