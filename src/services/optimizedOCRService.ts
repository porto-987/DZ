...
async function loadHuggingFaceModels() {
  ...
  try {
    isLoading = true;
    logger.info('OCR', '🔄 Chargement des modèles IA (cela peut prendre quelques secondes)...');
    ...
    logger.info('OCR', '✅ Modèles IA chargés avec succès');
    return huggingFaceCache;
  } catch (error) {
...
class OptimizedOCRService {
  ...
  private async extractTextFromImage(file: File): Promise<string> {
    try {
      ...
      logger.info('OCR', '🔄 Analyse OCR de l\'image...');
      const { data: { text } } = await Tesseract.recognize(file, 'fra');
      logger.info('OCR', '✅ OCR terminé');
  ...
  private async performAdvancedAnalysis(text: string, models: any): Promise<ExtractedData> {
    try {
      logger.info('OCR', '🤖 Analyse IA avancée en cours...');
      ...
      logger.info('OCR', '✅ Analyse IA terminée');
